...
SELECT EMP_ID, EMP_NAME, JOB_CODE, SALARY
FROM EMPLOYEE
    -- WHERE (JOB_CODE, SALARY) IN (('J1',8000000),
    --                              ('J2',3700000));
...
-- 3)
SELECT EMP_ID, EMP_NAME, NVL(DEPT_CODE,'없음') 부서코드, SALARY
FROM EMPLOYEE
...
-- 해결 방법 : ORDER BY로 이미 정렬한 테이블을 가지고 ROWNUM 순번을 매기면 됨.
...
SELECT ROWNUM, S.*/*"ROUND(AVG(SALARY))" , SALARY*/ 
...
-- 다만, 정렬되지 않은 상태에서는 순위를 매기면 의미가 없으므로 
-- 선 정렬 후 순위매기기를 해야한다. => 우선적으로 인라인뷰로 ORDER BY 절을 만들고 그 후에 메인 쿼리에서 순번을 붙임. 
...
/*
    ...
    정렬 기준 : ORDER BY절(정렬기준 컬럼이름, 오름차순/내림차순) , NULLFIRST나 NULLLAST는 기술 불가.
...
FROM EMPLOYEE; -- 공동 19위 2명 -> 21위

SELECT EMP_NAME, SALARY, DENSE_RANK() OVER(ORDER BY SALARY DESC) 순위
FROM EMPLOYEE; -- 공동 19위 2명, 그다음 순위는 20위 
