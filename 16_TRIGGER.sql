/*
    <트리거>
    내가 지정한 테이블에 INSERT, DELETE 등의 DML문에 의해 변경사항이 발생하였을때
    자동으로 매번 실행할 내용을 미리 정의해둘 수 있는 객체.
    
    EX)
    회원탈퇴시 기존의 회원테이블에 데이터 DELETE후 곧바로 탈퇴된 회원들만 따로 보관하는 테이블에 자동으로 INSERT처리해야된다.
    신고횟수가 일정 수를 넘었을때 묵시적으로 해당 회원을 블랙리스트 처리되게끔.
    입출고에 대한 데이터가 기록될때마다(INSERT될 때마다) 해당 상품에 대한 재고수량을 매번 수정(UPDATE)해야 될때 등.
...
-- 이력번호로 매번 새로운 번호를 발생시켜 들어갈 수 있게 도와주는 시퀀스 
...
-- TB_PRODUCT 테이블에서 200번 상품 10개를 증가시키는 UPDATE 문 실행
UPDATE TB_PRODUCT
    SET STOCK = STOCK+10
WHERE PCODE = 205;

 -- 205번상품이 오늘날짜로 20개 입고
 INSERT INTO TB_PRODETAIL
...
/*
    ...
    - 상품이 입고된경우 -> 해당상품을 찾아서 재고 수량을 증가 UPDATE +
    
    - 상품이 출고된경우 -> 해당상품을 찾아서 재고 수량 감소 UPDATE -
*/

CREATE OR REPLACE TRIGGER TRG_O1
AFTER INSERT ON TB_PRODETAIL
FOR EACH ROW 
BEGIN 
    -- 상품이 입고된 경우 -> 재고수량을 증가 시켜줘야 함 
    IF (:NEW.STATUS = '입고')
        THEN 
        UPDATE TB_PRODUCT
...
INSERT INTO TB_PRODETAIL
VALUES(SEQ_DCODE.NEXTVAL, 215, SYSDATE, 90, '입고');
