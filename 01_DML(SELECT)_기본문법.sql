-- DML : 데이터 조작, SELECT(DQL = DATE QUERY LANGUAGE) INSERT, UPDATE, DELETE
...
--- 4번. EMPLOYEE 테이블의 직원명, 이메일, 전화번호, 입사일 컬럼만 조회
SELECT 
    EMP_NAME,
    EMAIL,
    PHONE,
...
SELECT
    ...
    SALARY,
    BONUS,
    (BONUS*SALARY+SALARY) *12
FROM EMPLOYEE;
--> 산술연산 과정에서 NULL값이 존재할 경우 산술연산 결과마저도 NULL이 된다. 

-- EMPLOYEE 테이블로부터 직원명, 입사일, 근무일수 조회(오늘날짜 - 입사일) 조회
...
/*
    <컬럼명에 별칭 부여하기>
    [표현법]
    컬럼명 AS 별칭, 컬럼명 AS "별칭", 컬럼명 별칭, 컬럼병 "별칭"
    
    AS라는 키워드를 붙이든 안붙이든 간에 
    별칭에 특수문자나 띄어쓰기가 포함될 경우 ""를 묶어서 표기해줘야 함.
...
SELECT 
    EMP_NAME,
    SALARY AS "급여(월)",
    SALARY*12 AS "연봉(보너스 미포함)" -- "" 를 포함해야 괄호를 적을 수 있다
FROM EMPLOYEE;
...
/*
    ...
    '_' : 1글자
          ...
          비교대상 컬럼명 LIKE '__문자' => 컬럼값 중에 '문자' 앞에 무조건 2글자가 존재하는 경우 조회
*/  
...
-- 보너스를 받지 않는 사원들(보너스 컬럼의 값이 NULL)의 사번, 이름, 급여, 보너스 
...
/*
    <IN>
    비교 대상 컬럼 값에 내가 제시한 목록들 중 일치하는 값이 있는지 없는지 판단.
...
-- 직급코드가 J1, J3, J4가 아닌 사원들의 모든 컬럼 조회
SELECT *
FROM EMPLOYEE
...
/*
    ...
    컬럼과 리터럴(임의의 문자열 값 등)을 연결할 수 있음.
...
--------------------------------------------------------------------------------
  ...
  1  FROM 테이블명
  2  WHERE 조건식(생략가능)
  4  ORDER BY [정렬기준으로 세우고자 하는 컬럼명/별칭/컬럼순번] [ASC/DESC](생략가능) [NULLS FIRST/ NULLS LAST](생략가능)
...
-- ORDER BY BONUS DESC; -- DESC && NULLS FIRST 기본값.
...
ORDER BY 3; -- 3번째 열을 기준으로 기본 정렬 (ASC)

-- ORDER BY 절은 숫자뿐만 아니라 문자열이라던지 날짜에 대해서도 정렬 가능.
