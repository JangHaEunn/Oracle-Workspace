...
GROUP BY DEPT_CODE; -- 오류 남. 문법상 그룹함수를 WHERE절에서 쓸 수가 없다
...
SELECT JOB_CODE 직급코드, ROUND(AVG(SALARY)) 평균급여 , COUNT(*) 사원수, MAX(SALARY)최고급여, MIN(SALARY)최소급여
FROM EMPLOYEE
GROUP BY JOB_CODE
HAVING ROUND(AVG(SALARY))>=3000000;
...
/*
  ...
  6.  ORDER BY [정렬기준에 해당하는 컬럼명/ 별칭/ 컬럼의 순번] [ASC/DESC] 생략 가능 [NULLS FIRST/ NULLS LAST] (생략가능)
...
/*
    ...
    - MINUS (차집합) : 선행 쿼리문 결과값에서 후행 쿼리문 결과값을 뺀 나머지 부분
                     => 선행 쿼리문 결과값 - 교집합 1
    주의해야 할 점 : 두 쿼리문의 결과를 합쳐서 한개의 테이블로 보여줘야 하기 때문에 두 쿼리문의 SELECT 절 부분은 같아야 한다.
                  즉 조회할 "컬럼"이 동일해야 한다.
*/
...
-- 직급코드가 J6이거나 또는 부서코드가 D1인 사원들을 조회 (사번, 사원명, 부서코드, 직급코드 ) => UNION
...
SELECT EMP_ID, EMP_NAME, DEPT_CODE, JOB_CODE
FROM EMPLOYEE
WHERE DEPT_CODE = 'D1'; -- 차태연, 전지연만 나옴 
