...
/*
    ...
    DB에서 처리하는게 당연히 성능이 더 좋음.(데이터를 넘길때마다 네트워크 자원을 소비하게 됨)
    
    프로시저 단점
    1. DB자원을 직접 사용하기 때문에 DB에 부하를 주게 됨(남용하며 안된다)
    2. 관리적 측면에서 자바소스코드, 오라클 코드 동시에 형상관리하기가 어려움.
    
    정리)
    한번에 처리되는 데이터량이 많고 성능을 요구하는 처리는 대체로 자바보다는 DB상에서 처리하는것이
    성능 측면에서는 나을것이고 소스관리(유지보수) 즉 형상관리 측면에서는 자바가 더 좋다.
*/
--------------------------------------------------------------------------------
...
SELECT EMP_NAME, CALC_SALARY(EMP_ID) FROM EMPLOYEE;