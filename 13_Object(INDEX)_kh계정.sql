...
-- 실행계획 : DBMS가 특정 쿼리문을 실행하는데 있어서 실행할 계획 
SELECT * 
FROM EMPLOYEE
JOIN DEPARTMENT ON DEPT_CODE = DEPT_ID -- CARDINALITY 10 / 25
WHERE EMP_NAME = '선동일';
...
/*
    ...
    1) 조건절에 자주 등장하는 컬럼 
    2) 항상 = 으로 비교되는 컬럼
    3) 중복되는 데이터가 최소한인 컬럼 ( == 분포도가 높은) 
    4) ORDER BY 절에 자주 사용되는 컬럼
    5) JOIN조건으로 자주 사용되는 컬럼 
    
    인덱스의 장점
    
    1) WHERE 절에 인덱스의 컬럼을 사용하게 되면 훨씬 빠르게 연산할 수 있음.
    
    2) ORDER BY 연산을 사용할 필요가 없다(이미 정렬되어 있음)
    
    3) MIN, MAX 등의 집계함수로 값을 찾을 때 연산속도가 매우 빠름. (정렬되어 있기 때문에)
    
    인덱스의 단점 
    1) DML에 취약함. 
    INSERT, UPDATE, DELETE 등 데이터가 새롭게 추가, 삭제가 되면 인덱스 테이블 안에 있는 값들도 다시 정렬하고, 
    ROWID(물리적주소)값도 수정해줘야 함. 
    
    2) INDEX를 이용한 INDEX-SCAN보다 단순한 FULL-SCAN이 더 유리할때가 있다. 
