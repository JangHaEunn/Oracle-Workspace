...
-- 1) 컬럼추가 / 수정 / 삭제
-- 1_1) 컬럼추가 (ADD) :  ADD 추가할 컬럼명 자료형 DEFAULT 기본값 단, DEFAULT 기본값은 생략가능. 
...
-- 현재 변경하고자 하는 컬럼에 담겨있는 값보다 "작게"는 변경이 불가능하다. 
...
/*
    ...
    나만의 제약조건명 부여하고자 할 때 : CONSTRAINT 제약조건명을 제약 조건 앞에다가 추가. 
...
SELECT * FROM DEPT_COPY;
-- 3_1) 컬럼명변경 : RENAME COLUMN 기존컬럼명 TO 바꿀컬럼명 
-- DEPT_COPY 테이블에서 DEPT_TITLE 컬럼을 DEPT_NAME으로 바꾸기. 
ALTER TABLE DEPT_COPY RENAME COLUMN DEPT_TITLE TO DEPT_NAME; 

-- 3_2) 제약조건명 변경 : RENAME CONSTRAINT 기존제약명 TO 바꿀제약명 
ALTER TABLE DEPT_COPY RENAME CONSTRAINT SYS_C0011244 TO DCOPY_DID_NN;

-- 3_3) 테이블명 변경 : RENAME 기존테이블명 TO 바꿀테이블명 -> RENAME TO 바꿀 테이블명 
...
-- EMPLOYEE_COPY3 테이블에 외래키 제약조건 추가 
