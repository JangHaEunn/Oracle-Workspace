...
-- 접속계정 변경(SYSTEM 관리자)후 권한부여 

GRANT CREATE VIEW TO KH;

-- VIEW FROM절에서 테이블인것처럼 사용 가능. 
...
-- 뷰 특징
-- 1. 컬럼에 대해 산술연산 및 함수의 실행겨로가를 포함할 수 있다. 단, 반드시 별칭을 부여해야함.

CREATE VIEW V_EMP_SALARY
AS SELECT EMP_NAME, (SALARY+SALARY*NVL(BONUS,0)) * 12 
FROM EMPLOYEE;
...
/*
    INSERT, UPDATE, DELETE 수행할때 -> 가능하긴 한데 실제 테이블에 담겨있는 값이 변경됨.
    만약에 내가 업데이트 하고자 하는 컬럼이 가상 컬럼이라면 DML이 불가능함. 
*/
